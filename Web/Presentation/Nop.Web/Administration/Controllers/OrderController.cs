...
namespace Nop.Admin.Controllers
{
    public partial class OrderController : BaseAdminController
    {
        ...
        [HttpPost, ActionName("Edit")]
        [FormValueRequired("btnReceivePayment")]
        public virtual ActionResult DuePayment(int id)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageOrders))
                return AccessDeniedView();
            ...
            try
            {
                decimal paymentToday = Convert.ToDecimal(Request["txtAmount"].ToString().Trim()==string.Empty? "0": Request["txtAmount"].ToString());
                string dueDate = Request["txtDueDate"].ToString().Trim();

                string _query = "EXEC SP_InsertPayment @id=" + id + ",@paymentToday=" + paymentToday + ",@DueDate=";
                if (dueDate == "")
                {
                    _query += "null;";
                }
                ...
                //check for full paid 
                //_query = @"SELECT cast(max(o.OrderTotal)-SUM(od.[PaidToday]) as bigint) paid
                //        FROM[dbo].[Payment] od
                //            inner join[dbo].[Order] o
                //            on od.OrderId = o.Id
                //        where od.OrderId = " + id;
                _query = @"EXEC DueList @OrderId = " + id;

                DataTable dt = _dataOperation.GetDataTable(_query);
                if (dt.Rows.Count > 0)
                {
        ...
        public virtual ActionResult PdfChallan(int orderId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageOrders))
                return AccessDeniedView();
            ...
            using (var stream = new MemoryStream())
            {
                _pdfService.PrintOrdersToChallan(stream, orders, _orderSettings.GeneratePdfInvoiceInCustomerLanguage ? 0 : _workContext.WorkingLanguage.Id, vendorId);
                bytes = stream.ToArray();
            }
            return File(bytes, MimeTypes.ApplicationPdf, string.Format("Challan_{0}.pdf", order.Id));
        }
