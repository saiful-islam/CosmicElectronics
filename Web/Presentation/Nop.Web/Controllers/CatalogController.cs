...
namespace Nop.Web.Controllers
{
    public partial class CatalogController : BasePublicController
    {
        ...
        public ActionResult ProductBySKU()
        {
            try
            {
                string sku = Request["txtSKU"].ToString();
                if (sku.Trim() != "")
                {
                    string _query = "SELECT [Id] FROM [dbo].[Product] where [Sku]='" + sku+ "' and Deleted ='False' and Published='True'";
                    DataOperation _dataOperation = new DataOperation();
                    DataTable dt =_dataOperation.GetDataTable(_query);
                    if (dt.Rows.Count > 0)
                    {
                        return RedirectToAction("ProductDetails", "Product", new {productId = dt.Rows[0][0].ToString()});
                    }
                    return RedirectToAction("ProductDetails", "Product", new { productId = 0 });
                }
