...
namespace Nop.Services.Common
{
    ...
    public partial class PdfService : IPdfService
    {
        ...
        public virtual void PrintOrdersToPdf(Stream stream, IList<Order> orders, int languageId = 0, int vendorId = 0)
        {
            ...
            foreach (var order in orders)
            {
                ...
                if (vendorId == 0)
                {
                    ...
                    //Due
                    DataOperationForLib objData = new DataOperationForLib();
                    string strQuery = @"exec [dbo].[SP_OrderDue] @orderID=" + order.Id;
                    DataTable dt = objData.GetDataTable(strQuery);
                ...
                doc.Add(new Phrase(String.Format("-------------------------------------------------------------------------", font)));
                doc.Add(new Phrase(Environment.NewLine));

                //logo
                logoPicture = _pictureService.GetPictureById(pdfSettingsByStore.LogoPictureId);
                logoExists = logoPicture != null;

                //header
                headerTable = new PdfPTable(logoExists ? 2 : 1);
                headerTable.RunDirection = GetDirection(lang);
                headerTable.DefaultCell.Border = Rectangle.NO_BORDER;

                //store info
                store = _storeService.GetStoreById(order.StoreId) ?? _storeContext.CurrentStore;
                anchor = new Anchor(store.Url.Trim(new[] { '/' }), font);
                anchor.Reference = store.Url;

                cellHeader = new PdfPCell(new Phrase(String.Format(_localizationService.GetResource("PDFInvoice.Order#", lang.Id), order.CustomOrderNumber), titleFont));
                cellHeader.Phrase.Add(new Phrase(Environment.NewLine));
                ...
                var addressTable2 = new PdfPTable(2);
                addressTable2.RunDirection = GetDirection(lang);
                addressTable2.DefaultCell.Border = Rectangle.NO_BORDER;
                addressTable2.WidthPercentage = 100f;
                addressTable2.SetWidths(new[] { 50, 50 });

                //billing info
                var billingAddress2 = new PdfPTable(1);
                billingAddress2.DefaultCell.Border = Rectangle.NO_BORDER;
                billingAddress2.RunDirection = GetDirection(lang);

                billingAddress2.AddCell(new Paragraph(_localizationService.GetResource("PDFInvoice.BillingInformation", lang.Id), titleFont));

                if (_addressSettings.CompanyEnabled && !String.IsNullOrEmpty(order.BillingAddress.Company))
                    billingAddress2.AddCell(new Paragraph("   " + String.Format(_localizationService.GetResource("PDFInvoice.Company", lang.Id), order.BillingAddress.Company), font));

                billingAddress2.AddCell(new Paragraph("   " + String.Format(_localizationService.GetResource("PDFInvoice.Name", lang.Id), order.BillingAddress.FirstName + " " + order.BillingAddress.LastName), font));
                if (_addressSettings.PhoneEnabled)
                    billingAddress2.AddCell(new Paragraph("   " + String.Format(_localizationService.GetResource("PDFInvoice.Phone", lang.Id), order.BillingAddress.PhoneNumber), font));
                if (_addressSettings.FaxEnabled && !String.IsNullOrEmpty(order.BillingAddress.FaxNumber))
                    billingAddress2.AddCell(new Paragraph("   " + String.Format(_localizationService.GetResource("PDFInvoice.Fax", lang.Id), order.BillingAddress.FaxNumber), font));
                if (_addressSettings.StreetAddressEnabled)
                    billingAddress2.AddCell(new Paragraph("   " + String.Format(_localizationService.GetResource("PDFInvoice.Address", lang.Id), order.BillingAddress.Address1), font));
                if (_addressSettings.StreetAddress2Enabled && !String.IsNullOrEmpty(order.BillingAddress.Address2))
                    billingAddress2.AddCell(new Paragraph("   " + String.Format(_localizationService.GetResource("PDFInvoice.Address2", lang.Id), order.BillingAddress.Address2), font));
                if (_addressSettings.CityEnabled || _addressSettings.StateProvinceEnabled || _addressSettings.ZipPostalCodeEnabled)
                    billingAddress2.AddCell(new Paragraph("   " + String.Format("{0}, {1} {2}", order.BillingAddress.City, order.BillingAddress.StateProvince != null ? order.BillingAddress.StateProvince.GetLocalized(x => x.Name, lang.Id) : "", order.BillingAddress.ZipPostalCode), font));
                if (_addressSettings.CountryEnabled && order.BillingAddress.Country != null)
                    billingAddress.AddCell(new Paragraph("   " + order.BillingAddress.Country.GetLocalized(x => x.Name, lang.Id), font));

                //VAT number
                if (!String.IsNullOrEmpty(order.VatNumber))
                    billingAddress2.AddCell(new Paragraph("   " + String.Format(_localizationService.GetResource("PDFInvoice.VATNumber", lang.Id), order.VatNumber), font));

                //custom attributes
                var customBillingAddressAttributes2 = _addressAttributeFormatter.FormatAttributes(order.BillingAddress.CustomAttributes);
                if (!String.IsNullOrEmpty(customBillingAddressAttributes2))
                {
                    //TODO: we should add padding to each line (in case if we have sevaral custom address attributes)
                    billingAddress2.AddCell(new Paragraph("   " + HtmlHelper.ConvertHtmlToPlainText(customBillingAddressAttributes2, true, true), font));
                }
                ...
                if (vendorId == 0)
                {
                    ...
                    if (!String.IsNullOrEmpty(paymentMethodStr))
                    {
                        billingAddress2.AddCell(new Paragraph(" "));
                        billingAddress2.AddCell(new Paragraph("   " + String.Format(_localizationService.GetResource("PDFInvoice.PaymentMethod", lang.Id), paymentMethodStr), font));
                        billingAddress2.AddCell(new Paragraph());
                    }
                    ...
                    if (customValues != null)
                    {
                        ...
                        {
                            billingAddress2.AddCell(new Paragraph(" "));
                            billingAddress2.AddCell(new Paragraph("   " + item.Key + ": " + item.Value, font));
                            billingAddress2.AddCell(new Paragraph());
                        }
                    }
                }
                addressTable2.AddCell(billingAddress2);

                //shipping info
                var shippingAddress2 = new PdfPTable(1);
                shippingAddress2.DefaultCell.Border = Rectangle.NO_BORDER;
                shippingAddress2.RunDirection = GetDirection(lang);
                ...
                if (order.ShippingStatus != ShippingStatus.ShippingNotRequired)
                {
                    ...
                    if (!order.PickUpInStore)
                    {
                        ...
                        shippingAddress2.AddCell(new Paragraph(_localizationService.GetResource("PDFInvoice.ShippingInformation", lang.Id), titleFont));
                        if (!String.IsNullOrEmpty(order.ShippingAddress.Company))
                            shippingAddress2.AddCell(new Paragraph("   " + String.Format(_localizationService.GetResource("PDFInvoice.Company", lang.Id), order.ShippingAddress.Company), font));
                        shippingAddress2.AddCell(new Paragraph("   " + String.Format(_localizationService.GetResource("PDFInvoice.Name", lang.Id), order.ShippingAddress.FirstName + " " + order.ShippingAddress.LastName), font));
                        if (_addressSettings.PhoneEnabled)
                            shippingAddress2.AddCell(new Paragraph("   " + String.Format(_localizationService.GetResource("PDFInvoice.Phone", lang.Id), order.ShippingAddress.PhoneNumber), font));
                        if (_addressSettings.FaxEnabled && !String.IsNullOrEmpty(order.ShippingAddress.FaxNumber))
                            shippingAddress2.AddCell(new Paragraph("   " + String.Format(_localizationService.GetResource("PDFInvoice.Fax", lang.Id), order.ShippingAddress.FaxNumber), font));
                        if (_addressSettings.StreetAddressEnabled)
                            shippingAddress2.AddCell(new Paragraph("   " + String.Format(_localizationService.GetResource("PDFInvoice.Address", lang.Id), order.ShippingAddress.Address1), font));
                        if (_addressSettings.StreetAddress2Enabled && !String.IsNullOrEmpty(order.ShippingAddress.Address2))
                            shippingAddress2.AddCell(new Paragraph("   " + String.Format(_localizationService.GetResource("PDFInvoice.Address2", lang.Id), order.ShippingAddress.Address2), font));
                        if (_addressSettings.CityEnabled || _addressSettings.StateProvinceEnabled || _addressSettings.ZipPostalCodeEnabled)
                            shippingAddress2.AddCell(new Paragraph("   " + String.Format("{0}, {1} {2}", order.ShippingAddress.City, order.ShippingAddress.StateProvince != null ? order.ShippingAddress.StateProvince.GetLocalized(x => x.Name, lang.Id) : "", order.ShippingAddress.ZipPostalCode), font));
                        if (_addressSettings.CountryEnabled && order.ShippingAddress.Country != null)
                            shippingAddress2.AddCell(new Paragraph("   " + order.ShippingAddress.Country.GetLocalized(x => x.Name, lang.Id), font));
                        //custom attributes
                        var customShippingAddressAttributes = _addressAttributeFormatter.FormatAttributes(order.ShippingAddress.CustomAttributes);
                        if (!String.IsNullOrEmpty(customShippingAddressAttributes))
                        {
                            //TODO: we should add padding to each line (in case if we have sevaral custom address attributes)
                            shippingAddress2.AddCell(new Paragraph("   " + HtmlHelper.ConvertHtmlToPlainText(customShippingAddressAttributes, true, true), font));
                        }
                        shippingAddress2.AddCell(new Paragraph(" "));
                    }
                    else
                    if (order.PickupAddress != null)
                    {
                        shippingAddress2.AddCell(new Paragraph(_localizationService.GetResource("PDFInvoice.Pickup", lang.Id), titleFont));
                        if (!string.IsNullOrEmpty(order.PickupAddress.Address1))
                            shippingAddress2.AddCell(new Paragraph(string.Format("   {0}", string.Format(_localizationService.GetResource("PDFInvoice.Address", lang.Id), order.PickupAddress.Address1)), font));
                        if (!string.IsNullOrEmpty(order.PickupAddress.City))
                            shippingAddress2.AddCell(new Paragraph(string.Format("   {0}", order.PickupAddress.City), font));
                        if (order.PickupAddress.Country != null)
                            shippingAddress2.AddCell(new Paragraph(string.Format("   {0}", order.PickupAddress.Country.GetLocalized(x => x.Name, lang.Id)), font));
                        if (!string.IsNullOrEmpty(order.PickupAddress.ZipPostalCode))
                            shippingAddress2.AddCell(new Paragraph(string.Format("   {0}", order.PickupAddress.ZipPostalCode), font));
                        shippingAddress2.AddCell(new Paragraph(" "));
                    }
                    shippingAddress2.AddCell(new Paragraph("   " + String.Format(_localizationService.GetResource("PDFInvoice.ShippingMethod", lang.Id), order.ShippingMethod), font));
                    shippingAddress2.AddCell(new Paragraph());

                    addressTable2.AddCell(shippingAddress2);
                }
                else
                {
                    shippingAddress2.AddCell(new Paragraph());
                ...
                //products
                productsHeader = new PdfPTable(1);
                productsHeader.RunDirection = GetDirection(lang);
                productsHeader.WidthPercentage = 100f;
                cellProducts = new PdfPCell(new Phrase(_localizationService.GetResource("PDFInvoice.Product(s)", lang.Id), titleFont));
                cellProducts.Border = Rectangle.NO_BORDER;
                productsHeader.AddCell(cellProducts);
                doc.Add(productsHeader);
                doc.Add(new Paragraph(" "));
                ...
                productsTable = new PdfPTable(_catalogSettings.ShowSkuOnProductDetailsPage ? 5 : 4);
                productsTable.RunDirection = GetDirection(lang);
                productsTable.WidthPercentage = 100f;
                if (lang.Rtl)
                {
        ...
        /// <summary>
        /// Print orders to Chalan
        /// </summary>
        /// <param name="stream">Stream</param>
        /// <param name="orders">Orders</param>
        /// <param name="languageId">Language identifier; 0 to use a language used when placing an order</param>
        /// <param name="vendorId">Vendor identifier to limit products; 0 to to print all products. If specified, then totals won't be printed</param>
        public virtual void PrintOrdersToChallan(Stream stream, IList<Order> orders, int languageId = 0, int vendorId = 0)
        {
            if (stream == null)
                throw new ArgumentNullException("stream");
            ...
            foreach (var order in orders)
            {
                ...
                billingAddress.AddCell(new Paragraph("Customer Info:", titleFont));
                ...
                var productsTable = new PdfPTable(_catalogSettings.ShowSkuOnProductDetailsPage ? 4 : 4);
                productsTable.RunDirection = GetDirection(lang);
                productsTable.WidthPercentage = 100f;
                if (lang.Rtl)
                {
                ...
                foreach (var orderItem in orderItems)
                {
                    ...
                    //SL
                    SL++;
                    cellProductItem = new PdfPCell(new Phrase(SL.ToString(), font));
                    cellProductItem.HorizontalAlignment = Element.ALIGN_LEFT;
                    productsTable.AddCell(cellProductItem);
                ...
                if (vendorId == 0)
                {
                    ...
                    var pTotal = new PdfPCell(new Paragraph(String.Format("{0} {1}", "Total : ", totalQuantity), titleFont));
                    pTotal.HorizontalAlignment = Element.ALIGN_RIGHT;
                    pTotal.Border = Rectangle.NO_BORDER;
                    totalsTable.AddCell(pTotal);
                ...
                #region Footer
                    ...
                    var column1Lines = "Shop Manager                                                       Customer";
                    var column2Lines = "Director";
                    if (column1Lines.Any() || column2Lines.Any())
                    {
                        var totalLines = 1;
                        const float margin = 43;
                        ...
                        if (column1Lines.Any())
                        {
                            ...
                            column1.Border = Rectangle.NO_BORDER;
                            column1.HorizontalAlignment = Element.ALIGN_LEFT;
                            column1.Phrase.Add(new Phrase(column1Lines, font));
                            column1.Phrase.Add(new Phrase(Environment.NewLine));
                            footerTable.AddCell(column1);
                        }
                        else
                        ...
                        if (column2Lines.Any())
                        {
                            var column2 = new PdfPCell(new Phrase());
                            column2.Border = Rectangle.NO_BORDER;
                            column2.HorizontalAlignment = Element.ALIGN_RIGHT;
                            column2.Phrase.Add(new Phrase(column2Lines, font));
                            column2.Phrase.Add(new Phrase(Environment.NewLine));
                            footerTable.AddCell(column2);
                        }
                        else
